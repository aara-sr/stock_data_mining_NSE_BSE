"""
BSE Data Fetch Module
Fetches historical stock data for BSE-listed companies using yfinance
...
bse_logger = logging.getLogger('bse_logger')
if not bse_logger.handlers:
    bse_logger.setLevel(logging.INFO)
    handler = logging.FileHandler(os.path.join(log_dir, 'bse_fetch.log'))
    formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')
    handler.setFormatter(formatter)
    bse_logger.addHandler(handler)

# Also add console output
console_handler = logging.StreamHandler()
console_handler.setFormatter(logging.Formatter('%(asctime)s - %(levelname)s - %(message)s'))
bse_logger.addHandler(console_handler)


class BSEDataFetcher:
    """
    Fetches historical stock data for BSE-listed companies
    """
    
    def __init__(self, bse_list_path=None):
        """
        Initialize BSE Data Fetcher
        
        Args:
            bse_list_path (str): Path to the CSV file containing BSE stock list
        """
        self.bse_list_path = bse_list_path or os.path.join('BSE_LIST', 'BSEstocks.csv')
        self.stocks_df = None
        self.base_dir = os.path.join(os.getcwd(), 'BSE')
        os.makedirs(self.base_dir, exist_ok=True)
        
    def load_stocks_list(self):
        """Load the list of stocks from CSV"""
        try:
            # Read only the first four expected columns, ignoring trailing empty fields
            self.stocks_df = pd.read_csv(self.bse_list_path, usecols=[0, 1, 2, 3], dtype=str)
            self.stocks_df.columns = self.stocks_df.columns.str.strip()
            
            # Use Symbol column if available, otherwise fall back to Security Id
            if 'Symbol' in self.stocks_df.columns:
                self.stocks_df['Symbol'] = self.stocks_df['Symbol'].astype(str).str.strip()
            elif 'Security Id' in self.stocks_df.columns:
                self.stocks_df['Symbol'] = self.stocks_df['Security Id'].astype(str).str.strip()
            else:
                raise ValueError(f"Required column 'Symbol' not found. Available columns: {list(self.stocks_df.columns)}")
            
            bse_logger.info(f"Loaded {len(self.stocks_df)} stocks from BSE list")
            return self.stocks_df
        except Exception as e:
            bse_logger.error(f"Error loading BSE stocks list: {str(e)}")
            return None
    ...
    def fetch_stock_data(self, symbol, start_date=None, end_date=None, progress_callback=None):
        ...
        Args:
            symbol (str): Stock symbol/Security Id (e.g., 'ABB')
        ...
        try:
            ...
            bse_logger.info(f"Fetching data for {symbol} ({ticker}) from {start_date} to {end_date}")
            
            # Download historical data
            data = yf.download(ticker, start=start_date, end=end_date, progress=False)
            
            if data is None or data.empty:
                bse_logger.warning(f"No data found for {symbol}")
                return None
            ...
            try:
                ...
                bse_logger.warning(f"Could not fetch additional info for {symbol}: {str(e)}")
            ...
            bse_logger.info(f"Successfully fetched {len(data)} records for {symbol}")
            return data
            
        except Exception as e:
            bse_logger.error(f"Error fetching data for {symbol}: {str(e)}")
            return None
    ...
    def save_stock_data_consolidated(self, symbol, data):
        ...
        try:
            ...
            bse_logger.info(f"Saved consolidated data for {symbol} to {filename}")
            return True
            
        except Exception as e:
            bse_logger.error(f"Error saving consolidated data for {symbol}: {str(e)}")
            return False
    
    def fetch_all_stocks(self, start_date=None, end_date=None, progress_callback=None):
        """
        Fetch data for all stocks in the BSE list
        ...
        for idx, row in self.stocks_df.iterrows():
            symbol = row['Symbol']
            bse_logger.info(f"Processing {idx+1}/{len(self.stocks_df)}: {symbol}")
...
===============================================
"""
        bse_logger.info(summary)
        return summary


def get_bse_fetcher():
    """Factory function to get BSE fetcher instance"""
    return BSEDataFetcher()
...
if __name__ == "__main__":
    ...
    bse_logger.info("Starting BSE data fetch...")
